...
public class PresentLocatioinList extends ArrayAdapter<JSONObject>
{
    ...
    SharePrefsEntry sp;
    int callingCat;
    public PresentLocatioinList(Context con, ArrayList<JSONObject> l, int c)
    {
                ...
                public void onClick(View v) {
                    try
                    {
                        if(Integer.parseInt(jsonObject.getString("id")) > 0 )
                        {
                            ...
                            {
                                BaseActivity.currentLocSelected = jsonObject.getString("id");
                                try
                                {
                                    ProductsMani.location.setText(jsonObject.getString(title));
                                    ProductsMani.locid = jsonObject.getString("id");
                                }catch (Exception e){}

                                try
                                {
                                    ProductsManiExplicit.location.setText(jsonObject.getString(title));
                                    ProductsManiExplicit.locid = jsonObject.getString("id");
                                }catch (Exception e){}
                            }
                        }
